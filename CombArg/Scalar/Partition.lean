...
The pieces of the partition with `(piece j ∩ nearCritical).Nonempty`
are wrapped, together with their selected witness, into a
`FiniteCoverWithWitnesses`. Pieces failing the condition are mapped
to the empty set with a placeholder energy; their `saving_bound`
holds vacuously. The two-fold overlap and coverage bounds transfer
directly from `piece_multiplicity_le_two` and
`exists_piece_containing` via the conditional. -/

/-- Selected piece in the assembled cover: keeps `piece j` if it
intersects `nearCritical`, otherwise the empty set. -/
private noncomputable def selectedPiece
    (T : Finset ↥(nearCritical f m₀ N))
    (j : Fin ((endpts witness T).card - 1)) : Set unitInterval :=
  if (piece witness T j ∩ nearCritical f m₀ N).Nonempty
  then piece witness T j
  else ∅

/-- Selected replacement energy for the assembled cover: the energy of
a covering witness when `piece j` intersects `nearCritical`, else the
zero function. -/
private noncomputable def selectedEnergy
    (T : Finset ↥(nearCritical f m₀ N))
    (hT : nearCritical f m₀ N ⊆ ⋃ tk ∈ T, coverIvl witness tk)
    (j : Fin ((endpts witness T).card - 1)) : unitInterval → ℝ :=
  if h : (piece witness T j ∩ nearCritical f m₀ N).Nonempty
  then
    let tk := (exists_witness_for_piece witness hT
                h.choose_spec.1 h.choose_spec.2).choose
    (witness tk.val tk.property).replacementEnergy
  else fun _ => 0

/-- `selectedEnergy` is continuous: it is either a witness's
`replacementEnergy` (continuous by `LocalWitness.replacementEnergy_continuous`)
or the constant zero. -/
private lemma selectedEnergy_continuous
    (T : Finset ↥(nearCritical f m₀ N))
    (hT : nearCritical f m₀ N ⊆ ⋃ tk ∈ T, coverIvl witness tk)
    (j : Fin ((endpts witness T).card - 1)) :
    Continuous (selectedEnergy witness T hT j) := by
  unfold selectedEnergy
  by_cases h : (piece witness T j ∩ nearCritical f m₀ N).Nonempty
  · simp only [h, ↓reduceDIte]
    exact (witness _ _).replacementEnergy_continuous
  · simp only [h, ↓reduceDIte]
    exact continuous_const

/-- The saving bound holds on `selectedPiece`. In the non-empty branch,
the witness selection lemma places `piece j` inside
`val⁻¹(Icc (bounds tk).1 (bounds tk).2)`, where the witness saving holds
on the open `Ioo` interior; the bound extends to the closed `Icc`
preimage by `mem_closure_val_preimage_Ioo` plus
`IsClosed.closure_subset_iff` (the level set `{s | 1/(4N) ≤ f s − E s}`
is closed). In the empty branch, `selectedPiece j = ∅`, so the bound
is vacuous. -/
private lemma selectedEnergy_saving_bound
    (hf : Continuous f)
    (T : Finset ↥(nearCritical f m₀ N))
    (hT : nearCritical f m₀ N ⊆ ⋃ tk ∈ T, coverIvl witness tk)
    (j : Fin ((endpts witness T).card - 1)) :
    ∀ t ∈ selectedPiece witness T j,
      f t - selectedEnergy witness T hT j t ≥ 1 / (4 * (N : ℝ)) := by
  intro t ht
  unfold selectedPiece selectedEnergy at *
  by_cases h : (piece witness T j ∩ nearCritical f m₀ N).Nonempty
  · rw [if_pos h] at ht
    simp only [h, ↓reduceDIte]
    -- Pick witness tk and use saving extension to closure.
    let h_wit := exists_witness_for_piece witness hT h.choose_spec.1 h.choose_spec.2
    let tk := h_wit.choose
    have h_subset : piece witness T j ⊆
        Subtype.val ⁻¹' Set.Icc (bounds witness tk).1 (bounds witness tk).2 :=
      h_wit.choose_spec.2
    have h_t_in_Icc :
        t.val ∈ Set.Icc (bounds witness tk).1 (bounds witness tk).2 :=
      h_subset ht
    -- Bounds on (a, b) extracted from witness.
    ...
    -- Saving holds on val⁻¹(Ioo (bounds tk).1 (bounds tk).2).
    have h_sav_Ioo : ∀ s : unitInterval,
        s.val ∈ Set.Ioo (bounds witness tk).1 (bounds witness tk).2 →
          f s - (witness tk.val tk.property).replacementEnergy s ≥
            1 / (4 * (N : ℝ)) := by
      intro s hs
      exact (witness tk.val tk.property).saving_bound s
        (h_bspec.2.2 hs)
    -- Saving extends to closure: t ∈ val⁻¹(Icc) ⊆ closure(val⁻¹(Ioo)).
    have h_t_closure :
  ...
  · rw [if_neg h] at ht
    exact absurd ht (Set.notMem_empty _)

/-- The selected pieces cover `nearCritical`. -/
private lemma selectedPiece_covers
    (T : Finset ↥(nearCritical f m₀ N)) :
    nearCritical f m₀ N ⊆
      ⋃ j : Fin ((endpts witness T).card - 1), selectedPiece witness T j := by
  intro t ht
  obtain ⟨j, hj⟩ := exists_piece_containing witness T t
  refine Set.mem_iUnion.mpr ⟨j, ?_⟩
  unfold selectedPiece
  have h_inter : (piece witness T j ∩ nearCritical f m₀ N).Nonempty :=
    ⟨t, hj, ht⟩
  rw [if_pos h_inter]
  exact hj

/-- The selected pieces have multiplicity at most two: each is a subset
of the corresponding `piece`, and `piece_multiplicity_le_two` already
bounds those. -/
private lemma selectedPiece_twoFold
    (T : Finset ↥(nearCritical f m₀ N)) (t : unitInterval) :
    (Finset.univ.filter
      (fun j : Fin ((endpts witness T).card - 1) =>
        t ∈ selectedPiece witness T j)).card ≤ 2 := by
  refine le_trans (Finset.card_le_card ?_)
    (piece_multiplicity_le_two witness T t)
  intro j hj
  rw [Finset.mem_filter] at hj ⊢
  refine ⟨hj.1, ?_⟩
  unfold selectedPiece at hj
  by_cases h : (piece witness T j ∩ nearCritical f m₀ N).Nonempty
  · rw [if_pos h] at hj
    exact hj.2
  · rw [if_neg h] at hj
    exact absurd hj.2 (Set.notMem_empty _)
...
theorem exists_refinement_partition
    (hf : Continuous f) (_hm : m₀ = sSup (Set.range f)) (hN : 0 < N)
  ...
  have hM2 : 2 ≤ (endpts witness_data T).card :=
    endpts_card_ge_two witness_data T
  refine ⟨{
    ι := Fin ((endpts witness_data T).card - 1)
    ιFintype := inferInstance
    nonempty := ⟨⟨0, by omega⟩⟩
    piece := selectedPiece witness_data T
    replacementEnergy := selectedEnergy witness_data T hT
    ...
    saving_bound :=
      selectedEnergy_saving_bound witness_data hf T hT
    covers_delta_near_critical := selectedPiece_covers witness_data T
    twoFold := selectedPiece_twoFold witness_data T }⟩
