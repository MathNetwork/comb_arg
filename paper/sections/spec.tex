...
\node[tierhead, above=4pt of T1.north] {%
  \textbf{Tier 1 \,---\, \texttt{CombArg.OneDim}} \\[-1pt]
  {\scriptsize\itshape DLT-faithful path}
};
...
\node[tierhead, above=4pt of T2.north] {%
  \textbf{Tier 2 \,---\, \texttt{CombArg.Scalar}} \\[-1pt]
  {\scriptsize\itshape cheap path}
};
...
(\texttt{CombArg.OneDim}) follows the DLT \S3.2 Step~1 algorithm
in three stages culminating in the structured \texttt{DLTCover}
(\textbf{bold} box), which preserves the spacing, $\sigma$-injectivity,
and partial-refinement invariants needed by a future geometric
lift to a modified sweepout (\texttt{CombArg/Geometric/}, not yet
implemented; the lift consumes \texttt{DLTCover}, not the
abstract output below). Tier~2 (\texttt{CombArg.Scalar}) proves
the same conclusion directly by partition by sorted endpoints
and imports none of Tier~1's spacing / refinement / parity
machinery --- the dependency-graph fact behind
