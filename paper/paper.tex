...
\definecolor{defcolor}{HTML}{2B6CB0}       % blue — definitions
\definecolor{thmcolor}{HTML}{805AD5}       % purple — theorems/propositions/lemmas
\definecolor{remarkcolor}{HTML}{718096}    % gray — remarks
\definecolor{leantag}{HTML}{2B6CB0}        % Lean ref — blue

% Shared tcolorbox base style: thin gray outer border, colored left bar.
\tcbset{
  thmbox/.style={
    enhanced, breakable,
    colback=white,
...
% Definition (blue bar), numbered X.Y within section.
\newtcbtheorem[number within=section]{definition}{Definition}
  {thmbox, colframe=defcolor}{def}
% Theorem / Proposition / Lemma (purple bar), share definition's counter.
\newtcbtheorem[use counter from=definition]{theorem}{Theorem}
  {thmbox, colframe=thmcolor}{thm}
\newtcbtheorem[use counter from=definition]{proposition}{Proposition}
  {thmbox, colframe=thmcolor}{prop}
\newtcbtheorem[use counter from=definition]{lemma}{Lemma}
  {thmbox, colframe=thmcolor}{lem}
% Remark (gray bar).
\newtcbtheorem[use counter from=definition]{remark}{Remark}
  {thmbox, colframe=remarkcolor}{rem}
...
% Inline Lean reference: small blue typewriter decl name.
%   \leanref{CombArg.PairableCover}
\newcommand{\leanref}[1]{{\color{leantag}\ttfamily\small #1}}
...
\begin{theorem}{Almgren--Pitts min-max theorem~\cite{Pit81,SS81}}{minmax}
...
the Almgren--Pitts combinatorial argument, stated in the
Colding--De~Lellis formulation as follows.

\begin{proposition}{Almgren--Pitts combinatorial Argument, \cite[Prop.~3.4]{CDL03}}{AP-combinatorial}
...
Stripped of its GMT instantiation, the content of Steps~1--2 is
the following scalar statement, which is the precise theorem we
formalize.

\begin{theorem}{Extracted scalar version of Steps 1--2}{intro-scalar}
...
The boundary $\partial \Omega_t$ is replaced by a continuous
...
$E_t$; the almost-minimizing hypothesis disappears. What remains
is the combinatorial bookkeeping. Theorem~\ref{thm:intro-scalar}
reappears in \S\ref{sec:app-spec} as the library's public
one-parameter API (Theorem~\ref{thm:main}); it is a specialization
of an abstract core (Theorem~\ref{thm:core}) parametric in the
sweepout space and in a pair $(\delta, \varepsilon)$ replacing the
hardcoded $(1/N, 1/(4N))$.
...
\begin{definition}{\texttt{PairableCover} class}{pcover}
\leanref{CombArg.PairableCover}
...
\begin{definition}{\texttt{LocalWitness} structure}{witness}
\leanref{CombArg.LocalWitness}
...
\begin{definition}{Near-critical set}{NC}
\leanref{CombArg.Refinement.nearCritical}
...
\begin{definition}{\texttt{FiniteCoverWithWitnesses} structure}{ref}
\leanref{CombArg.FiniteCoverWithWitnesses}
...
This is the abstract form of the refined covering
$\{\overline{J_l}\}$ produced at the end of~\cite[\S3.2 Step 1]{DLT13};
conditions (II)--(III) are DLT's two-fold overlap and per-piece
saving invariants.
\end{definition}

\begin{definition}{\texttt{InitialCover} structure}{ic}
\leanref{CombArg.Refinement.InitialCover}
...
be near-critical.} Corresponds to the initial covering $\{I_i\}$
of $\NC$ entering~\cite[\S3.2]{DLT13} \emph{before} the Step~1
refinement.
\end{definition}

\begin{definition}{\texttt{PartialRefinement} structure}{pr}
\leanref{CombArg.Refinement.PartialRefinement}
...
This captures the \emph{mid-induction state} of~\cite[\S3.2
Step~1]{DLT13}, where each $I_j$ has been either kept whole
(``the same'') or split (``into two halves'').
\end{definition}

\subsection{Public API: abstract core}
\label{sec:core-spec}

\begin{theorem}{Abstract core}{core}
\leanref{CombArg.exists\_sup\_reduction\_of\_cover}
\noindent
...
\begin{theorem}{One-parameter application}{main}
\leanref{CombArg.exists\_sup\_reduction}
\noindent
...
each one mirroring a segment of~\cite[\S3.2]{DLT13}:

\begin{center}\small
\begin{tabular}{@{}lll@{}}
\toprule
\textsc{our stage} & \textsc{DLT13 \S3.2 segment} & \textsc{output}\\
\midrule
Stage A & (setup, pre-Step~1) & \texttt{InitialCover} \\
Stage B & Step~1 (refinement of the covering) & terminal \texttt{PartialRefinement} \\
Stage C & Step~2 (conclusion: bookkeeping) & \texttt{FiniteCoverWithWitnesses} \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Theorem~\ref{thm:core} then consumes the output of Stage~C and
closes the chain.

\subsection{Stage A: existence of the initial cover}

\begin{proposition}{Existence of an initial cover}{exists-ic}
\leanref{CombArg.Refinement.exists\_initialCover}
...
This stage realizes~\cite[\S3.2 Step~1]{DLT13}: each initial
interval $I_j$ with $j \ge 2$ is either kept whole (``remains
the same'') or split (``into two halves''), depending on whether
the paired cover at $t_{\sigma(\ell)}$ is compatible with the new
$t_{i^*}$. Figure~\ref{fig:step1} reproduces the
\cite[Fig.~1]{DLT13} schematic.
...
\begin{tikzpicture}[
  ...
  gridline/.style={dashed,gray!55,line width=0.4pt}
]
% J_l bars, stacked vertically (staircase), in DLT Fig 1 left style.
% Each bar sits at its own y-row; adjacent J_l overlap on the x-axis
% to show the two-fold invariant.
\def\ya{5}  \def\yb{4}  \def\yc{3}  \def\yd{2}  \def\ye{1}

% vertical dashed guides at overlap boundaries
\foreach \x in {2.5, 3.5, 4.8, 6.5, 7.8}
  \draw[gridline] (\x,-0.7) -- (\x,\ya+0.35);

% J_1
\node[lbl,anchor=east] at (-0.2,\ya) {$J_1$};
\draw[bar] (0.3,\ya) -- (2.5,\ya);
\draw[tick] (0.3,\ya-0.12) -- (0.3,\ya+0.12);
\draw[tick] (2.5,\ya-0.12) -- (2.5,\ya+0.12);

% J_2 (overlaps J_1 on the left)
\node[lbl,anchor=east] at (-0.2,\yb) {$J_2$};
\draw[bar] (1.9,\yb) -- (3.5,\yb);
\draw[tick] (1.9,\yb-0.12) -- (1.9,\yb+0.12);
\draw[tick] (3.5,\yb-0.12) -- (3.5,\yb+0.12);

% J_3 (overlaps J_2)
\node[lbl,anchor=east] at (-0.2,\yc) {$J_3$};
\draw[bar] (3.0,\yc) -- (4.8,\yc);
\draw[tick] (3.0,\yc-0.12) -- (3.0,\yc+0.12);
\draw[tick] (4.8,\yc-0.12) -- (4.8,\yc+0.12);

% J_4 (overlaps J_3)
\node[lbl,anchor=east] at (-0.2,\yd) {$J_4$};
\draw[bar] (4.3,\yd) -- (6.5,\yd);
\draw[tick] (4.3,\yd-0.12) -- (4.3,\yd+0.12);
\draw[tick] (6.5,\yd-0.12) -- (6.5,\yd+0.12);

% J_5 (overlaps J_4)
\node[lbl,anchor=east] at (-0.2,\ye) {$J_5$};
\draw[bar] (6.0,\ye) -- (7.8,\ye);
\draw[tick] (6.0,\ye-0.12) -- (6.0,\ye+0.12);
\draw[tick] (7.8,\ye-0.12) -- (7.8,\ye+0.12);

% parameter axis [0,1]
\draw[-{Latex[length=2mm]},line width=0.8pt] (-0.2,-0.3) -- (8.4,-0.3);
\node[lbl,right=2pt] at (8.4,-0.3) {$t$};
\draw[tick] (0.0,-0.3-0.12) -- (0.0,-0.3+0.12);
\draw[tick] (8.0,-0.3-0.12) -- (8.0,-0.3+0.12);
\node[lbl,below=1pt] at (0.0,-0.4) {$0$};
\node[lbl,below=1pt] at (8.0,-0.4) {$1$};

% NC bracket underneath the axis
\draw[line width=1pt,gray!70!black]
  (0.8,-0.9) -- (7.2,-0.9);
\node[lbl,below=1pt] at (4.0,-0.95) {$\NC \;=\; \{t : f(t) \ge m_0 - 1/N\}$};

\end{tikzpicture}
\caption{\label{fig:step1}Staircase layout of the refined cover
$\{J_l\}$ on $[0,1]$, after~\cite[Fig.~1 left]{DLT13}. Each piece
$J_l$ is drawn on its own row so that vertical sweeps are legible:
at any parameter $t \in [0,1]$, the number of rows the vertical
line crosses is the overlap multiplicity; the cover satisfies
$|\{l : t \in \overline{J_l}\}| \le 2$ for every $t$ (two-fold
invariant). The $\NC$ bracket on the parameter axis marks the
$1/N$-near-critical set, which is contained in
$\bigcup_l J_l$.}
\end{figure}

Formally, $\{J_l\}$ is built by induction from $\mathrm{step\_zero}$
(base case) via $\mathrm{step\_succ\_at}$ (inductive step).

\begin{definition}{Base case}{step0}
\leanref{CombArg.Refinement.step\_zero}
$\mathrm{step\_zero}(\mathrm{ic})$ is the length-$1$ partial
refinement with
$J_1 := I_1$ and $\sigma(1) := 1$.
\end{definition}

\begin{definition}{Inductive step}{step-succ}
\leanref{CombArg.Refinement.step\_succ\_at}
...
\begin{proposition}{Termination}{exists-terminal}
\leanref{CombArg.Refinement.exists\_terminal\_refinement}
...
This stage realizes~\cite[\S3.2 Step~2]{DLT13}: from the refined
$\{J_l\}$ together with the local-witness saving bound one packages
a \texttt{FiniteCoverWithWitnesses} (Definition~\ref{def:ref}),
whose three invariants (per-piece saving floor, two-fold overlap,
coverage of $\NC$) are precisely the data DLT's Step 2 bookkeeping
consumes. Here we extend the saving bound from open $J_k$ to
$\overline{J_k}$ and derive the two-fold invariant from
spacing~(a). Theorem~\ref{thm:core} then turns the assembled cover
into the sup-reducing competitor $f'$ (our formalization stops
here; DLT proceeds to derive a contradiction against
$m_0 = \inf_\Lambda \mathcal{F}$, which is the caller's
responsibility).

\begin{proposition}{TwoFold via parity}{twofold}
\leanref{CombArg.Refinement.terminal\_twoFold}
...
(\leanref{CombArg.Refinement.InitialCover.chain\_spacing},
\leanref{CombArg.Refinement.InitialCover.closure\_disjoint\_of\_even\_gap}).
...
(\leanref{CombArg.Refinement.InitialCover.not\_three\_overlap}).
\end{proof}

\begin{proposition}{Closure extension of saving bound}{saving-bound}
\leanref{CombArg.Refinement.saving\_bound\_closure}
...
\begin{lemma}{Assembly}{assembly}
\leanref{CombArg.Refinement.exists\_refinement}
