...
Then there exist a finite family of closed intervals
...
Let $X$ be a pseudo-metric space with a \texttt{PairableCover X}
instance (Definition~\ref{def:pcover}). Let $f : \unit \to
\mathbb{R}$ be continuous with $m_0 = \sup f$ and $m_0 > 0$, and
$N \in \mathbb{N}$ with $N > 0$. Suppose every $t \in \NC$
admits a \texttt{LocalWitness} at $t$ with saving $1/(4N)$
(Definition~\ref{def:witness}). Then there exists a
\texttt{FiniteCoverWithWitnesses}$(\unit, f, m_0, 1/N, 1/(4N))$
(Definition~\ref{def:ref}): a finite family of closed intervals
