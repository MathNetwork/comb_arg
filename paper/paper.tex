...
This section is for downstream consumers --- authors building
their own geometric-measure-theoretic min-max formalization in
Lean --- who want to plug \CombArg{} in as the combinatorial
step. The library treats De~Lellis--Tasnady's
Lemma~3.1~\cite{DLT13} (the geometric replacement step) as
\emph{abstract input}: it asks for a
\texttt{LocalWitness} (Definition~\ref{def:witness}) at every
near-critical parameter and gives back a scalar sup reduction
(Corollary~\ref{thm:main}). What the GMT-side formalization
must produce is exactly the data that fits
the four fields of \texttt{LocalWitness}.
...
\item A continuous boundary energy
  $f : \unit \to \mathbb{R}$ --- the consumer's sweepout's
  energy as a function of the path parameter.
...
If the consumer's admissible class $\mathcal{A}$ has the
characterization
...
The pattern below adapts \texttt{examples/MinimalUsage.lean}
in the repository to a hypothetical GMT consumer:
