...
% Colored Lean declaration tag for theorem/definition/proposition
% headers and inline proof-body references. Renders the Lean decl
% path in a restrained blue typewriter font.
\definecolor{leantag}{HTML}{2B6CB0}
\newcommand{\lean}[1]{{\color{leantag}\texttt{#1}}}
...
\begin{definition}[\texttt{PairableCover} class, \lean{CombArg.PairableCover}]
\label{def:pcover}
...
\begin{definition}[\texttt{LocalWitness} structure, \lean{CombArg.LocalWitness}]
\label{def:witness}
...
\begin{definition}[Near-critical set, \lean{CombArg.nearCritical}]
\label{def:NC}
...
\begin{definition}[\texttt{FiniteCoverWithWitnesses} structure, \lean{CombArg.FiniteCoverWithWitnesses}]
\label{def:ref}
...
\begin{definition}[\texttt{InitialCover} structure, \lean{CombArg.Refinement.InitialCover}]
\label{def:ic}
...
\begin{definition}[\texttt{PartialRefinement} structure, \lean{CombArg.Refinement.PartialRefinement}]
\label{def:pr}
...
\begin{theorem}[Abstract core, \lean{CombArg.exists\_sup\_reduction\_of\_cover}]
\label{thm:core}
\leavevmode\par\noindent
...
\begin{theorem}[One-parameter application, \lean{CombArg.exists\_sup\_reduction}]
\label{thm:main}
\leavevmode\par\noindent
...
\begin{proposition}[Compactness of $\NC$, \lean{CombArg.Refinement.isCompact\_nearCritical}, \lean{CombArg.Refinement.nearCritical\_nonempty}]
...
\begin{proposition}[Existence of an initial cover, \lean{CombArg.Refinement.exists\_initialCover}]
...
\begin{definition}[Base case, \lean{CombArg.Refinement.step\_zero}]
\label{def:step0}
...
\begin{definition}[Inductive step, \lean{CombArg.Refinement.step\_succ\_at}]
\label{def:step-succ}
...
\begin{proposition}[Termination, \lean{CombArg.Refinement.exists\_terminal\_refinement}]
...
\begin{proposition}[TwoFold via parity, \lean{CombArg.Refinement.terminal\_twoFold}]
\label{prop:twofold}
...
(\lean{CombArg.Refinement.InitialCover.chain\_spacing},
\lean{CombArg.Refinement.InitialCover.closure\_disjoint\_of\_even\_gap}).
...
(\lean{CombArg.Refinement.InitialCover.not\_three\_overlap}).
\end{proof}

\begin{proposition}[Closure extension of saving bound, \lean{CombArg.Refinement.saving\_bound\_closure}]
...
\begin{lemma}[Assembly, \lean{CombArg.Refinement.exists\_refinement}]
\label{lem:assembly}
