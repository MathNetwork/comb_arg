...
functional. \me{The construction, due to Almgren and Pitts, is a
\emph{min-max} scheme on varifolds; regularity of the output is
due to Pitts \cite{Pit81} and Schoen--Simon \cite{SS81}.}
...
\node[box, minimum width=130mm] (core) at (0, 8) {%
  \texttt{exists\_refinement} \ (combinatorial core, $K = [0,1]$)
};
\node[tierlbl] at (-7, 8) {Tier 1\\combinatorial core};

% Tier 2: sup-reduction corollaries (side by side, explicit text widths)
\node[box, text width=48mm] (app) at (-4.5, 6) {%
  \texttt{exists\_sup\_reduction} \\ ($K = [0,1]$)
};
\node[box, text width=58mm] (genc) at ( 4.5, 6) {%
...
\node[decl] (D2b) at (0, 2.0) {step\_succ\_at};
\node[decl] (D2c) at (0, 1.5) {ExtendResult};
\node[decl] (D2d) at (0, 1.0) {exists\_terminal\_refinement};
\node[decl] (D2e) at (0, 0.5) {PartialRefinement};
...
\node[decl] (TF) at (0, -0.7) {twoFold invariant};

% Tier 5: disjointness sub-module (on SkippedSpacedIntervals)
\node[decl] (SP) at (0, -1.9) {SkippedSpacedIntervals};
\node[decl] (J1) at (-5.2, -2.8) {chain\_spacing};
\node[decl] (J2) at ( 0,   -2.8) {I\_closure\_disj\_of\_even\_gap};
\node[decl] (J3) at ( 5.2, -2.8) {not\_three\_overlap};
\node[tierlbl] at (-7, -2.35) {Tier 5\\disjointness};
...
\end{tikzpicture}
\caption{\label{fig:arch}\CombArg{} v0.2 architecture. The
...
\section{Use as a software dependency}
\label{sec:integration}

Suppose the min-max setting from the literature has been
formalized in Lean: an admissible class $\mathcal{A}$ of
sweepouts, the inf-sup characterization
$m_0 = \inf_{\Omega \in \mathcal{A}} \sup f_\Omega$, the local
replacement construction (De~Lellis--Tasnady's
Lemma~3.1~\cite{DLT13}), and the Hausdorff measure of boundary
surfaces. This section describes how the abstract declarations
of \CombArg{} then \emph{lift into the combinatorial step of
the original proof} from the literature.

The library is the formal abstract version of the combinatorial
slice that \cite{Pit81, CDL03, DLT13, DLR17, MN14} each
re-derive in their own notation. The Lean declarations do not
change between abstract and instantiated form --- they are
exactly the same definitions either way. What changes is that
the min-max setting supplies the \texttt{LocalWitness} inputs
against which the abstract declarations evaluate, and
\lean{exists\_sup\_reduction}{SupReduction.lean} then \emph{is}
the sup-reduction step of the original proof: the step that
produces a strictly-better sweepout $\Omega'$ contradicting the
inf-sup characterization of $m_0$.

\subsection{What the library already gives}
\label{sec:integration-library-side}

The combinatorial side is the abstract content already present
in this repository, with no GMT references:

\begin{itemize}
\item \texttt{LocalWitness}\,$(K, f, t, \varepsilon)$
  (Definition~\ref{def:witness}) --- the abstract per-parameter
  local reducer data.
\item \lean{exists\_refinement}{Refinement/Assembly.lean} ---
  the formal counterpart of DLT~\S3.2 Step~1 (interval
  refinement to a finite cover with two-fold overlap).
\item \lean{exists\_sup\_reduction\_of\_cover}{Core.lean} ---
  the formal counterpart of DLT~\S3.2 Step~2 (scalar
  bookkeeping over the cover).
\item \lean{exists\_sup\_reduction}{SupReduction.lean} --- the
  chained one-parameter combinatorial step.
\end{itemize}

\subsection{What the min-max setting must supply}
\label{sec:integration-supplies}

Three pieces of GMT-side data lift the abstract library to the
original proof:

\begin{itemize}
\item The continuous boundary energy
  $f := \Omega.\texttt{boundaryEnergy} : \unit \to \mathbb{R}$
  of a sweepout $\Omega$ realizing $m_0 = \sup f$.
\item A near-criticality parameter $N \in \mathbb{N}$ chosen so
  that $1/(4N)$ beats whatever contradiction window the
  admissible class requires.
\item The local replacement lemma: at every $t$ with
  $f(t) \ge m_0 - 1/N$, a
  $\texttt{LocalWitness}\,(\unit, f, t, 1/(4N))$ whose four
  fields come from the GMT-side replacement family.
\end{itemize}
...
Each field of \texttt{LocalWitness} lifts to a specific piece
of the GMT-side replacement data:

\begin{itemize}
\item \texttt{neighborhood} : DLT's open interval
  $(a_i, b_i) \subseteq [0,1]$ around $t$ on which the local
  replacement saves energy.
\item \texttt{isOpen\_neighborhood} and \texttt{t\_mem} :
  openness of the interval; the parameter $t$ lies in it.
\item \texttt{replacementEnergy} :
  $s \mapsto \mathcal{H}^n(\partial \widetilde{\Omega}_s)$,
  the boundary energy of the \emph{replaced} sweepout, where
  $\widetilde{\Omega}_s$ is the original sweepout with the
  local replacement inserted at parameter $s$.
\item \texttt{replacementEnergy\_continuous} : continuity of
  $s \mapsto \mathcal{H}^n(\partial \widetilde{\Omega}_s)$.
  DLT treats this implicitly through the continuity of the
  replaced family in the inserted parameter; the GMT side
  discharges it explicitly.
\item \texttt{saving\_bound} : the quantitative DLT~3.1
  inequality
  $f(s) - \mathcal{H}^n(\partial \widetilde{\Omega}_s) \ge 1/(4N)$
  for every $s \in (a_i, b_i)$. This is the substantive
  non-combinatorial content.
\end{itemize}

\subsection{What the lift produces: the literature's sup-reduction step}
\label{sec:integration-receives}

With the instantiation in place,
\lean{exists\_sup\_reduction}{SupReduction.lean} returns a
scalar competitor $f' : \unit \to \mathbb{R}$ with a
modification set $S \subseteq \unit$ satisfying

\begin{enumerate}
\item[(coverage)]
  $\{t \in \unit : f(t) \ge m_0 - 1/N\} \subseteq S$,
\item[(a)] pointwise dominance: $f'(t) \le f(t)$ for all $t$,
\item[(b)] localization: $f'(t) = f(t)$ for $t \notin S$,
\item[(c)] sup reduction:
  $\sup_{t \in \unit} f'(t) \le m_0 - 1/(4N)$.
\end{enumerate}

This is --- definitionally --- the sup-reduction step the
original proof uses. The competitor $f'$ and modification set
$S$ are the formal images of \emph{the sweepout's energy after
the local replacements have been combined}. Conditions (a) and
(b) anchor $f'$ to $f$ so that $f'$ is a genuine combinatorial
reduction of $f$, not a trivial constant.

\subsection{Closing the proof: the contradiction step}
\label{sec:integration-contradiction}

The literature now closes by lifting $f'$ back to a sweepout
$\Omega' \in \mathcal{A}$ and contradicting the inf-sup. In
Lean:

\begin{enumerate}
\item \lean{exists\_sup\_reduction}{SupReduction.lean} (this
  library) supplies $f'$ with $\sup f' \le m_0 - 1/(4N)$.
\item The min-max setting's lift takes $f'$ together with its
  modification set $S$ back to a sweepout
  $\Omega' \in \mathcal{A}$. The (b) localization guarantee
  makes the lift mechanical: $\Omega'$ agrees with $\Omega$
  outside $S$ and inserts the local replacement on each piece
  of $S$.
\item $\sup f_{\Omega'} \le m_0 - 1/(4N) < m_0
  = \inf_{\mathcal{A}} \sup$ contradicts
  $\Omega' \in \mathcal{A}$.
\end{enumerate}
...
\subsection{Skeleton client code}
\label{sec:integration-skeleton}

The pattern below shows how a downstream GMT formalization
would invoke \CombArg{}; it adapts
\texttt{examples/MinimalUsage.lean} in the repository, with
\texttt{YourGMT.*} stubs standing in for the GMT-side
declarations a consumer formalization would supply:

\begin{verbatim}
import CombArg
import YourGMT  -- consumer's formalization
...
theorem minmax_contradiction
  ...
  -- Lean image of DLT Lemma 3.1: per-parameter local witness.
  ...
  -- Lift f' back to a sweepout in admissibleClass.
...
Three identifiers carry the consumer's GMT-side
responsibility:

\begin{itemize}
\item \texttt{YourGMT.choose\_N} --- pick $N$ large enough
  that $1/(4N)$ beats the contradiction window the admissible
  class requires.
\item \texttt{YourGMT.localWitness\_of\_DLT} --- the
  formalization of DLT Lemma~3.1 (the substantive geometric
  work, returning a per-$t$ \texttt{LocalWitness}).
\item \texttt{YourGMT.lift\_sweepout} --- turn the scalar
  $f'$ together with its modification set $S$ back into a
  sweepout in $\mathcal{A}$.
\end{itemize}

\subsection{Known friction}
\label{sec:integration-friction}

\begin{itemize}
\item \textbf{1D parameter space only.} \CombArg{} is
  currently specialized to $K = \unit$. Multi-parameter
  sweepouts ($K = [0,1]^m$, in particular Almgren cycles) need
  a multi-parameter cover construction; planned for v0.3.
\item \textbf{Output is scalar, not geometric.} The consumer
  receives $f'$ with a sup bound and the modification set
  $S$; the lift back to a sweepout in $\mathcal{A}$ is the
  consumer's work. The (b) off-$S$ agreement is designed to
  make this lift mechanical.
\item \textbf{Continuity of \texttt{replacementEnergy}.}
  Discharged on the consumer's side. DLT~3.1 supplies it
  implicitly through the continuity of the replacement family
  in the inserted parameter; the Lean-level proof obligation
  falls on the consumer.
\item \textbf{Lean / Mathlib version pin.} \CombArg{} is on
  \texttt{leanprover/lean4:v4.30.0-rc2} with Mathlib pinned
  via \texttt{lake-manifest.json}. Bumps on either side may
  require coordination.
\item \textbf{Axiom budget.} \CombArg{} uses only
  \texttt{propext}, \texttt{Classical.choice}, and
  \texttt{Quot.sound}. Standard Mathlib measure-theory work
