...
\begin{tikzpicture}[
  ...
  decl/.style={box, font=\scriptsize\ttfamily, minimum width=42mm, minimum height=4.2mm, inner ysep=1.5pt},
...
\node[decl] (TF) at (0, -0.1) {twoFold invariant};

% Tier 5: disjointness sub-module (on SkippedSpacedIntervals)
\node[decl] (SP) at (0, -1.0) {SkippedSpacedIntervals};
\node[decl] (J1) at (-5.2, -1.9) {chain\_spacing};
\node[decl] (J2) at ( 0,   -1.9) {I\_closure\_disj\_of\_even\_gap};
\node[decl] (J3) at ( 5.2, -1.9) {not\_three\_overlap};
\node[tierlbl] at (-7, -1.45) {Tier 5\\disjointness};
...
re-derive in their own notation. Its four public declarations
make no GMT references and stand for the four pieces the
literature recombines at every instantiation.
\texttt{LocalWitness}\,$(K, f, t, \varepsilon)$
...
$f := \Omega.\texttt{boundaryEnergy} : \unit \to \mathbb{R}$ of
a sweepout $\Omega$ realizing $m_0 = \sup f$; a
near-criticality parameter $N \in \mathbb{N}$ chosen so that
$1/(4N)$ beats whatever contradiction window the admissible
class needs; and a local replacement lemma producing, at every
parameter $t$ with $f(t) \ge m_0 - 1/N$, a
$\texttt{LocalWitness}\,(\unit, f, t, 1/(4N))$ whose data
fields come from the GMT-side replacement family. The library's
Lean declarations do not change between the abstract and
instantiated forms --- they are exactly the same definitions
either way; what changes is that this min-max-side data flows
in as the inputs.
...
\subsection{The proof chain: from sup reduction to contradiction}
\label{sec:integration-chain}

With this data in place,
\lean{exists\_sup\_reduction}{SupReduction.lean} returns a
scalar competitor $f' : \unit \to \mathbb{R}$ together with a
modification set $S \subseteq \unit$ satisfying
$\{t \in \unit : f(t) \ge m_0 - 1/N\} \subseteq S$ (coverage),
the pointwise dominance $f'(t) \le f(t)$ everywhere~(a), the
localization $f'(t) = f(t)$ for $t \notin S$~(b), and the sup
reduction $\sup_{t} f'(t) \le m_0 - 1/(4N)$~(c). This is ---
definitionally --- the sup-reduction step the original proof
uses. The competitor $f'$ and modification set $S$ are the
formal images of \emph{the sweepout's energy after the local
replacements have been combined}; conditions (a) and (b) anchor
$f'$ to $f$ so that $f'$ is a genuine combinatorial reduction
of $f$ rather than a trivial constant.

The literature now closes the proof by lifting $f'$ back to a
sweepout $\Omega' \in \mathcal{A}$ and contradicting the
inf-sup. In Lean this chain has three pieces, of which the
library discharges the first:
\lean{exists\_sup\_reduction}{SupReduction.lean} produces $f'$
with $\sup f' \le m_0 - 1/(4N)$. The min-max setting then
...
makes this lift mechanical, since $\Omega'$ agrees with
$\Omega$ outside $S$ and inserts the local replacement on each
piece of $S$. The contradiction follows immediately from
$\sup f_{\Omega'} \le m_0 - 1/(4N) < m_0
= \inf_{\mathcal{A}} \sup$ together with
$\Omega' \in \mathcal{A}$. Together with the library's first
piece, these reconstruct line for line the literature's
contradiction chain.
...
The three \texttt{YourGMT.*} identifiers mark the GMT-side
...
$K = \unit$; multi-parameter sweepouts ($K = [0,1]^m$, in
particular Almgren cycles) will need a multi-parameter cover
construction, planned for v0.3. The output is scalar rather
than geometric: the consumer receives $f'$ with its sup bound
and the modification set $S$, and must lift back to a sweepout
...
only the three standard foundational axioms (\texttt{propext},
\texttt{Classical.choice}, \texttt{Quot.sound}), and standard
Mathlib measure-theory work stays within the same three.
