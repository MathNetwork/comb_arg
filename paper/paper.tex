...
% Lean community purple (logo colour) for all tags.
\definecolor{leanpurple}{HTML}{6F2DA8}
% Inline clickable tag (use in prose / bullet lists):
%   \lean{short_name}{File/Path.lean}
\newcommand{\lean}[2]{\href{\leanrepo/#2}{%
  \color{leanpurple}\texttt{#1}}}
% Margin tag (use right after \label{...} in theorem / def / prop):
%   \leanmargin{short_name}{File/Path.lean}
\newcommand{\leanmargin}[2]{%
  \marginnote{\scriptsize\color{leanpurple}\ttfamily
    \href{\leanrepo/#2}{#1}}%
}
% Multi-decl margin tag (two decls, stacked):
%   \leanmargintwo{a}{fileA}{b}{fileB}
\newcommand{\leanmargintwo}[4]{%
  \marginnote{\scriptsize\color{leanpurple}\ttfamily
    \href{\leanrepo/#2}{#1}\\[1pt]%
    \href{\leanrepo/#4}{#3}}%
}
