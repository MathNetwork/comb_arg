...
DLT start by applying~\cite[Lemma~3.1]{DLT13} to produce, at each
near-critical parameter $t$, a replacement family
...
$U_t$ plays $(a_i, b_i)$, and we collapse DLT's separate
``saving on the inner core'' bound $-\tfrac{1}{2N}$ and ``no-loss
outside'' bound $+\tfrac{1}{4N}$ into a single uniform saving
$\varepsilon = \tfrac{1}{4N}$ that holds everywhere on $U_t$.
...
Next DLT observe that if $t \in (a_i, b_i) \cap (a_j, b_j)$, then
$j = i+1$, and moreover $\mathrm{dist}(U_i', U_{i+1}') > 0$. This
...
DLT then define a modified sweepout $\{\partial \Omega'_t\}$
...
DLT's two-piece blend on overlaps.
...
specializes to DLT's arithmetic; the abstract core reads off the
same algebra for arbitrary $(\delta, \varepsilon)$ with
$\varepsilon \le \delta$.
