...
infrastructure for an Almgren--Pitts min-max construction has
been carried out --- in particular, the Hausdorff measure of
boundary surfaces, the local-replacement construction
(De~Lellis--Tasnady's Lemma~3.1~\cite{DLT13}), and an admissible
class of sweepouts. This section describes how \CombArg{}
integrates with such a formalization as the combinatorial step.

The library treats the GMT side as \emph{abstract input}: it
asks for a \texttt{LocalWitness} (Definition~\ref{def:witness})
at every near-critical parameter and gives back a scalar sup
reduction (Corollary~\ref{thm:main}). The interface is
project-agnostic: the same \texttt{LocalWitness} shape works
for any GMT formalization, whether a one-off proof in a single
repository or a community-scale Mathlib extension. What the
GMT-side formalization must produce is exactly the data that
fits the five fields of \texttt{LocalWitness}.

\subsection{What the consumer supplies}
\label{sec:integration-supplies}

For one invocation of
\lean{exists\_sup\_reduction}{SupReduction.lean}, the consumer
supplies:

\begin{itemize}
\item A continuous boundary energy
  $f : \unit \to \mathbb{R}$ --- the sweepout's energy as a
  function of the path parameter.
\item A scalar $m_0 \in \mathbb{R}$ with
  $m_0 = \sup_{t \in \unit} f(t)$. In a min-max setting, this
  is the level at which the sweepout sits.
\item A near-criticality parameter $N \in \mathbb{N}$ with
  $N > 0$. The quantitative output is $1/(4N)$.
\item For every $t \in \unit$ with $f(t) \ge m_0 - 1/N$, a
  $\texttt{LocalWitness}\,(\unit, f, t, 1/(4N))$.
\end{itemize}

\subsection{The \texttt{LocalWitness} -- DLT~3.1 dictionary}
\label{sec:integration-dictionary}

The five fields of a \texttt{LocalWitness}
(Definition~\ref{def:witness}) translate directly to the data
DLT's Lemma~3.1 produces in the GMT setting:

\begin{itemize}
\item \texttt{neighborhood} : open interval $(a_i, b_i)
  \subseteq [0,1]$ around $t$ on which the local replacement
  saves energy.
\item \texttt{isOpen\_neighborhood} and \texttt{t\_mem} :
  openness of the interval; the parameter $t$ lies in it.
\item \texttt{replacementEnergy} : the boundary energy of the
  \emph{replaced} sweepout,
  $s \mapsto \mathcal{H}^n(\partial \widetilde{\Omega}_s)$,
  where $\widetilde{\Omega}_s$ is the consumer's sweepout with
  the local replacement inserted at parameter $s$.
\item \texttt{replacementEnergy\_continuous} : continuity of
  the above map in $s$. DLT treats this implicitly via the
  continuity of the replacement family in the inserted
  parameter; the consumer must produce the Lean-level proof.
\item \texttt{saving\_bound} : the quantitative DLT~3.1
  inequality
  $f(s) - \texttt{replacementEnergy}(s) \ge 1/(4N)$ for every
  $s \in (a_i, b_i)$. This is the substantive
  non-combinatorial content the consumer must establish.
\end{itemize}

\subsection{What the consumer receives}
\label{sec:integration-receives}

A scalar competitor $f' : \unit \to \mathbb{R}$ together with
a modification set $S \subseteq \unit$ satisfying
...
Conditions (a) and (b) anchor $f'$ to $f$: without them, the
constant $f' \equiv c$ for any $c \le m_0 - 1/(4N)$ would
satisfy (c) vacuously.

\subsection{Plugging into a min-max contradiction}
\label{sec:integration-contradiction}

If the admissible class $\mathcal{A}$ has the characterization
$m_0 = \inf_{\Omega \in \mathcal{A}} \sup f_\Omega$ and the
sweepout $\Omega$ realizes the inf with $f = f_\Omega$, the
contradiction is three steps:

\begin{enumerate}
\item Apply \lean{exists\_sup\_reduction}{SupReduction.lean}
  to obtain $f'$ with $\sup f' \le m_0 - 1/(4N)$.
\item Lift $f'$ back to a sweepout $\Omega' \in \mathcal{A}$
  --- consumer's GMT-side responsibility. The (b) localization
  guarantee is designed to make this lift routine: $\Omega'$
  agrees with $\Omega$ outside $S$ and uses the local
  replacement inside.
...
The library discharges step~1 in full; steps~2 and~3 are
domain-specific.
