...
\begin{proposition}[Almgren--Pitts combinatorial Argument,
{\cite[Prop.~3.4]{CDL03}}]
...
\begin{theorem}[Extracted scalar version of Steps 1--2]
...
% top row: initial cover {I_i} on a number line
\draw[-{Latex[length=2mm]}] (-0.3,3) -- (10.5,3);
\foreach \x/\name/\pos in {0.6/I_1/above, 2.5/I_2/above,
                           4.4/I_3/above, 6.3/I_4/above,
                           8.2/I_5/above}{%
  \draw[int] (\x,3) -- (\x+1.4,3);
  \node[lbl,\pos=3pt] at (\x+0.7,3) {$\name$};
  % small ticks at endpoints
  \draw[tick] (\x,3-0.08) -- (\x,3+0.08);
  \draw[tick] (\x+1.4,3-0.08) -- (\x+1.4,3+0.08);
}
% witness centers t_i (dots below the line)
\foreach \x/\name in {1.3/t_1, 3.2/t_2, 5.1/t_3, 7.0/t_4, 8.9/t_5}{%
  \filldraw (\x,3) circle (0.04);
  \node[lbl,below=2pt] at (\x,3) {$\name$};
}
\node[font=\scriptsize,anchor=west] at (-0.3,3.6) {initial cover $\{I_i\}$};

% bottom row: refined cover {J_l} -- I_2 split, others kept
\draw[-{Latex[length=2mm]}] (-0.3,0) -- (10.5,0);
% J_1 = I_1
\draw[int,red!60!black] (0.6,0) -- (2.0,0);
\draw[tick,red!60!black] (0.6,-0.08) -- (0.6,0.08);
\draw[tick,red!60!black] (2.0,-0.08) -- (2.0,0.08);
\node[lbl,above=3pt] at (1.3,0) {$J_1$};
% I_2 is split into J_2 (smaller) and J_3 (smaller) — the split point
\draw[int,red!60!black] (2.5,0) -- (3.3,0);
\draw[tick,red!60!black] (2.5,-0.08) -- (2.5,0.08);
\draw[tick,red!60!black] (3.3,-0.08) -- (3.3,0.08);
\node[lbl,above=3pt] at (2.9,0) {$J_2$};
\draw[int,red!60!black] (3.5,0) -- (3.9,0);
\draw[tick,red!60!black] (3.5,-0.08) -- (3.5,0.08);
\draw[tick,red!60!black] (3.9,-0.08) -- (3.9,0.08);
\node[lbl,above=3pt] at (3.7,0) {$J_3$};
% J_4 = I_3
\draw[int,red!60!black] (4.4,0) -- (5.8,0);
\draw[tick,red!60!black] (4.4,-0.08) -- (4.4,0.08);
\draw[tick,red!60!black] (5.8,-0.08) -- (5.8,0.08);
\node[lbl,above=3pt] at (5.1,0) {$J_4$};
% J_5 = I_4
\draw[int,red!60!black] (6.3,0) -- (7.7,0);
\draw[tick,red!60!black] (6.3,-0.08) -- (6.3,0.08);
\draw[tick,red!60!black] (7.7,-0.08) -- (7.7,0.08);
\node[lbl,above=3pt] at (7.0,0) {$J_5$};
% J_6 = I_5
\draw[int,red!60!black] (8.2,0) -- (9.6,0);
\draw[tick,red!60!black] (8.2,-0.08) -- (8.2,0.08);
\draw[tick,red!60!black] (9.6,-0.08) -- (9.6,0.08);
\node[lbl,above=3pt] at (8.9,0) {$J_6$};
\node[font=\scriptsize,anchor=west] at (-0.3,-0.5) {refinement $\{J_l\}$};

% dashed vertical showing correspondence of split
\draw[dashed,gray] (2.5,3) -- (2.5,0);
\draw[dashed,gray] (3.9,3) -- (3.9,0);

\end{tikzpicture}
\caption{\label{fig:step1}Refinement of the covering on $[0,1]$,
after~\cite[Fig.~1]{DLT13}. Top: the initial cover $\{I_i\}$ with
witness centers $t_i \in \NC$. Bottom: the refined cover
$\{J_l\}$; here $I_2$ is split into $J_2 \cup J_3$ because the
paired cover at $t_3$ intersects the one at $t_2$, while
$I_1, I_3, I_4, I_5$ are kept whole. In the resulting cover every
$t \in [0,1]$ lies in at most two of the $\overline{J_l}$
(two-fold invariant).}
\end{figure}
