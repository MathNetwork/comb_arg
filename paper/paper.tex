...
Under the hypotheses of Theorem~\ref{thm:intro-scalar}, there
exists $f' : [0,1] \to \mathbb{R}$ with
\[
...
(Definition~\ref{def:ref}), there exists $f' : K \to \mathbb{R}$
with
\[
...
\begin{proof}
Set $f'(t) := f(t) - \sum_{l \,:\, t \in p_l} s_l$ and verify the
hypotheses of Lemma~\ref{lem:scalar-sup-reduction}. Condition~(i):
each $s_l > 0$, so $f'(t) \le f(t)$. Condition~(ii): suppose
$f(t) \ge m_0 - \delta$. By (IV) of Definition~\ref{def:ref},
$t \in p_l$ for some $l$; by (III), $s_l \ge \varepsilon$. The
remaining savings in the sum (at most one more, by the two-fold
overlap (II)) are non-negative, so $f(t) - f'(t) \ge \varepsilon$.
The lemma yields $\sup f' \le m_0 - \varepsilon$.
\end{proof}

\begin{corollary}[One-parameter sup reduction]
\label{thm:main}
\leanmargin{exists\_sup\_reduction}{SupReduction.lean}
Under the hypotheses of Theorem~\ref{thm:core}, there exists
$f' : \unit \to \mathbb{R}$ with
\[
\sup f' \;\le\; m_0 - \tfrac{1}{4N}.
\]
\end{corollary}
