...
Lemma~3.1 produces a family $\{\partial \Omega_{i,t}\}$ with
\begin{itemize}\setlength{\itemsep}{1pt}\small
\item $\Omega_{i,t} = \Omega_t$ if $t \notin (a_i, b_i)$, and
      $\Omega_{i,t} \setminus U_i' = \Omega_t \setminus U_i'$
      if $t \in (a_i, b_i)$;
\item $\mathcal{H}^n(\partial \Omega_{i,t}) \le
...
\end{itemize}
If $t \in (a_i, b_i) \cap (a_j, b_j)$, then $j = i+1$ and
$\mathrm{dist}(U_i', U_{i+1}') > 0$. Define the modified sweepout
\begin{itemize}\setlength{\itemsep}{1pt}\small
\item $\Omega'_t = \Omega_t$ if $t \notin \bigcup_i J_i$;
\item $\Omega'_t = \Omega_{i,t}$ if $t$ is in a single $J_i$;
\item $\Omega'_t = [\Omega_t \setminus (U_i' \cup U_{i+1}')]
      \cup [\Omega_{i,t} \cap U_i']
      \cup [\Omega_{i+1,t} \cap U_{i+1}']$
      on $J_i \cap J_{i+1}$.
\end{itemize}
Bounding the area: for $t \notin K$ the point lies in at most
two $J_i$'s, so
\[
\mathcal{H}^n(\partial \Omega'_t) \;\le\;
\mathcal{H}^n(\partial \Omega_t) + \tfrac{1}{2N};
\]
for $t \in K$ the point lies in at least one
$(a_i + \delta, b_i - \delta)$ and in at most one other $J_l$,
so area gains $\ge \tfrac{1}{2N}$ inside some $U_i'$ and loses
$\le \tfrac{1}{4N}$ inside $U_l'$, hence
\[
\mathcal{H}^n(\partial \Omega'_t) \;\le\;
\mathcal{H}^n(\partial \Omega_t) - \tfrac{1}{4N}.
\]
