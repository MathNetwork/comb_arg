...
to Schoen--Simon \cite{SS81} in higher dimensions. [wording needs to be modified: Birkhoff is a classical and much simpler construction of a different space (and works for geodesics in any codimension), so misleading to state it like this; regularity is due to both works of Pitts and Schoen-Simon, what's written is not quite historically accurate]
...
The proof of Theorem~\ref{thm:AP-combinatorial} reduces to
 ...
 [Is that correct? DLT13 only proves existence for 1-parameter;
 i don't think argument for p-parameter follows from argument
 for 1-parameter (although they are similar)]
...
would be produced in the ambient GMT setting).
...
The arithmetic heart of the abstract core is a short lemma
independent of covers, pieces, or multiplicity: any competitor
that saves a fixed amount on the relevant super-level set
yields the same amount of supremum reduction.
...
about lowering the supremum of a bounded real-valued function
by exhibiting a competitor with a pointwise saving on the
...
super-level set. In the combinatorial argument, the competitor
$g$ is constructed piecewise from per-parameter local witnesses;
Theorem~\ref{thm:core} records how such data delivers the
...
(Definition~\ref{def:witness}). Then there exists $f' : \unit
\to \mathbb{R}$ with
\[
\sup f' \;\le\; m_0 - \tfrac{1}{4N}.
\]
...
Theorem~\ref{thm:main} is the specialization of
Theorem~\ref{thm:core} to $K = \unit$, $\delta = 1/N$,
$\varepsilon = 1/(4N)$; the 1D cover-refinement of
\S\ref{sec:proof} turns the local-witness data into the
\texttt{FiniteCoverWithWitnesses} required by the core.
...
(Tier 5) is an abstract \texttt{SkippedSpacedIntervals} structure
whose three lemmas combine into the two-fold overlap invariant
